...
const ChatbotAcademico = () => {
    ...
    return (
        <div className="animate-fadeIn w-full max-w-[1600px] mx-auto h-[calc(100vh-140px)] min-h-[600px] grid grid-cols-1 lg:grid-cols-4 gap-6 p-4">
            ...
            <div className="lg:col-span-3 flex flex-col h-full bg-white rounded-3xl border border-slate-200 shadow-xl overflow-hidden relative">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-xl font-black text-slate-800 tracking-tight flex items-center gap-2">
                                Asistente Académico
                                <span className="px-2 py-0.5 bg-emerald-100 text-emerald-700 text-[10px] rounded-full font-bold uppercase tracking-wider">Online</span>
                            </h1>
                            <p className="text-slate-500 font-medium text-xs">Resuelvo tus dudas sobre el pensum</p>
                        </div>
                    </div>
                ...
                {/* Messages Container */}
                <div ref={mensajesContainerRef} className="flex-1 overflow-y-auto p-4 md:p-8 space-y-6 custom-scrollbar bg-slate-50/50">
                    {mensajes.map((mensaje, index) => {
                        ...
                        return (
                            <div key={index} className={`flex items-start gap-3 ${!esBot ? 'flex-row-reverse' : ''} animate-fadeIn`}>
                                <div className={`w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center shadow-sm shrink-0 ${esBot ? 'bg-sky-500 text-white' : 'bg-slate-800 text-white'}`}>
                                ...
                                <div className={`max-w-[85%] md:max-w-[75%] px-5 py-3.5 rounded-2xl relative group shadow-sm text-sm md:text-base leading-relaxed ${esBot
                                        ...
                                        : 'bg-sky-600 text-white rounded-tr-none shadow-sky-200'
                                    }`}>

                                    {esBot && esUltimo ? <EfectoMecanografia texto={mensaje.texto} /> : <span className="whitespace-pre-wrap">{mensaje.texto}</span>}

                                    <div className={`text-[10px] mt-1.5 font-bold opacity-60 flex justify-end ${esBot ? 'text-slate-400' : 'text-sky-100'}`}>
                                    ...
                                    {esBot && (
                                        <button
                                            ...
                                            className="absolute -right-8 top-1 p-1.5 opacity-0 group-hover:opacity-100 transition-opacity text-slate-400 hover:text-sky-600 bg-white rounded-full shadow-sm"
                                            title="Copiar"
                                        >
                ...
                <div className="p-4 md:p-6 bg-white border-t border-slate-100">
                    <form onSubmit={enviarMensaje} className="relative flex gap-3 max-w-4xl mx-auto">
                        <input
                            ...
                            className="flex-1 pl-6 pr-4 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-sky-100 focus:border-sky-400 outline-none text-slate-700 placeholder:text-slate-400 transition-all shadow-inner"
                            disabled={cargando}
                        />
                        <button
                            type="submit"
                            disabled={cargando || !inputMensaje.trim()}
                            className="px-6 md:px-8 bg-sky-600 text-white rounded-2xl font-bold hover:bg-sky-700 disabled:opacity-50 transition-all flex items-center gap-2 shadow-lg shadow-sky-200 active:scale-95 group"
                        >
                ...
                {mostrarScrollTop && (
                    <button
                        onClick={scrollToTop}
                        className="absolute bottom-28 right-8 p-3 bg-white text-sky-600 rounded-full shadow-lg border border-sky-100 hover:bg-sky-50 transition-all z-10"
                    >
                        <ArrowUp size={20} />
                    </button>
                )}
            ...
            <div className="lg:col-span-1 flex flex-col gap-6 h-full overflow-y-auto custom-scrollbar pb-4">
                    ...
                    <div className="flex flex-col gap-2">
                        {preguntasSugeridas.map((pregunta, idx) => (
                            <button
                                ...
                                className="text-left px-4 py-3 text-xs md:text-sm bg-slate-50 border border-slate-100 text-slate-600 rounded-xl hover:bg-sky-50 hover:text-sky-700 hover:border-sky-200 transition-all font-medium active:scale-98"
                            >
                                {pregunta}
                            </button>
                        ))}
                ...
                {/* Widget 2: Tips */}
                <div className="bg-gradient-to-br from-sky-500 to-blue-600 rounded-3xl p-6 text-white shadow-lg shadow-sky-200 relative overflow-hidden">
                ...
                {/* Widget 3: Help / About */}
                <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm mt-auto">
                    <div className="flex items-center gap-3 mb-3">
                        <div className="p-2 bg-emerald-100 text-emerald-600 rounded-lg">
                            <Sparkles size={18} />
                        </div>
                        <div>
                            <h4 className="font-bold text-slate-800 text-sm">IA Académica</h4>
                            <p className="text-[10px] text-slate-400">Versión 2.0.0</p>
                        </div>
                    </div>
                    ...
                    <button
                        ...
                        className="mt-4 w-full py-2.5 bg-slate-50 text-slate-600 text-xs font-bold rounded-xl border border-slate-200 hover:bg-slate-100 transition-all"
                    >
                        Ver Documentación
                    </button>
                </div>
            ...
            {mostrarModalLimpiar && (
                <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4 animate-fadeIn">
                    <div className="bg-white rounded-3xl p-8 max-w-sm w-full shadow-2xl text-center transform scale-100 transition-all">
                        ...
                        <h3 className="text-xl font-black text-slate-800 mb-2">¿Limpiar conversación?</h3>
                        <p className="text-slate-500 text-sm mb-8">Se borrará todo el historial actual. No podrás deshacerlo.</p>
                        <div className="flex gap-3">
                            <button onClick={() => setMostrarModalLimpiar(false)} className="flex-1 py-3 bg-slate-100 text-slate-600 font-bold rounded-xl hover:bg-slate-200 transition-all">Cancelar</button>
                            <button onClick={confirmarLimpiar} className="flex-1 py-3 bg-rose-500 text-white font-bold rounded-xl hover:bg-rose-600 shadow-lg shadow-rose-200 transition-all">Limpiar</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            {mostrarModalAyuda && (
                <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4 animate-fadeIn">
                    <div className="bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl relative overflow-hidden">
                        ...
                        <button onClick={() => setMostrarModalAyuda(false)} className="absolute top-6 right-6 p-2 text-slate-300 hover:text-slate-600 transition-colors"><X size={24} /></button>
                        ...
                        <div className="flex items-center gap-4 mb-6">
                            ...
                            <div>
                                <h3 className="text-xl font-black text-slate-800">Guía de Uso</h3>
                                <p className="text-xs text-sky-600 font-bold uppercase tracking-wider">Capacidades del Bot</p>
                            </div>
                        </div>
                        ...
                        <div className="space-y-3 mb-8">
                            {[
                                ...
                                <div key={i} className="flex items-center gap-3 p-3 bg-slate-50 rounded-xl border border-slate-100">
                                    <item.icon size={16} className="text-sky-500" />
                                    <span className="text-sm text-slate-600 font-medium">{item.text}</span>
                                </div>
                            ))}
                        </div>

                        <div className="bg-sky-50 p-4 rounded-xl border border-sky-100 text-sky-800 text-xs font-medium leading-relaxed mb-6">
                            <strong>💡 Tip Pro:</strong> Intenta preguntar por códigos específicos para obtener respuestas más precisas al instante.
                        </div>

                        <button onClick={mostrarAyuda} className="w-full py-3.5 bg-sky-600 text-white font-bold rounded-xl hover:bg-sky-700 shadow-lg shadow-sky-200 transition-all active:scale-95">
